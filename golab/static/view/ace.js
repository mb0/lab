...
function previousToken(session, pos, type, ignored) {
	var r = pos.row, i = pos.column
	var toks, tok, skip = _.isArray(ignored);
	for (toks = null; i >= 0 || r > 0; i--) {
		if (toks === null || i < 0) {
			toks = session.getTokens(i < 0 ? --r : r);
			i = toks.length -1;
		}
		if (i < 0)
			continue;
		...
		if (tok.type == type)
			return tok;
		if (!skip || !_.contains(ignored, tok.type))
			return null;
	}
	return null;
}
